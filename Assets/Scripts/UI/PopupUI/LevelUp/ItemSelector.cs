...
{
    public static List<ItemData> GetRandomSelectableItems(RunData run, ItemDatabase database, int count)
    {
    ...
    private static bool IsSelectable(RunData run, ItemData item)
    {
        ...
        if (item is PassiveData passive)
            return level < passive.MaxLevel;
